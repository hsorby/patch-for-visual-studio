...
{
	int e = errno;
	struct quoting_options *p = (struct quoting_options *)malloc (sizeof *p);
...
struct quoting_options const *options)
{
	...
	static struct slotvector slotvec0 = {sizeof slot0, slot0};
	static struct slotvector *slotvec = &slotvec0;
	...
	if (nslots <= n0)
	{
		...
		if (slotvec == &slotvec0)
		{
			slotvec = (struct slotvector *)malloc (sizeof *slotvec);
			*slotvec = slotvec0;
		}
		slotvec = (struct slotvector *)realloc (slotvec, s);
...

	{
		...
		if (size <= qsize)
		{
			...
			slotvec[n].val = val = (char *)realloc (val == slot0 ? 0 : val, size);
