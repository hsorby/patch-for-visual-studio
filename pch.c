	...
	open_patch_file (char const *filename)
{
	...
	{
		pfp = fopen (filename, binary_transput ? "rb" : "rt");
		if (!pfp)
...
{
	if (!p_line)
		p_line = (char **) malloc((size_t)hunkmax * sizeof *p_line);
	if (!p_len)
		p_len = (size_t *) malloc((size_t)hunkmax * sizeof *p_len);
	if (!p_Char)
		p_Char = (char *)malloc ((size_t)hunkmax * sizeof *p_Char);
}
	...
	grow_hunkmax (void)
{
	...
	if ((p_line = (char **) realloc (p_line, (size_t)hunkmax * sizeof (*p_line)))
		&& (p_len = (size_t *) realloc (p_len, (size_t)hunkmax * sizeof (*p_len)))
		&& (p_Char = (char *) realloc (p_Char, (size_t)hunkmax * sizeof (*p_Char))))
		return true;
	if (!using_plan_a)
		memory_fatal ();
